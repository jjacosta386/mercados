...
function validarFormFechas(idFechaDesde, idFechaHasta, formato, nombreForm, anioRest) {
    ...
    var fechaActual2 = f.getFullYear() + "-" + (f.getMonth() + 1) + "-" + f.getDate();
    ...
    if (cadenaFechaIni == null || cadenaFechaIni.length < 1
        ...
        if (formato.search("-") > -1) {
            ...
            fecha3 = fechaConGuion(fechaActual2);
        } else {
        ...
        if (fechaDiff < 0) {
            ...
            if (fechaDiff > 1.001 && anioRest) {
                ...
                fechaDiff = fechaActual.getTime() - fecha1.getTime();
                fechaDiff = fechaDiff / (3600000 * 12 * 365.25);
                if (fechaDiff < 0) {
...
//                alert("Fecha Actual " + fechaActual.getTime() + " fecha1 " + fecha1.getTime() + " fecha diferencia " + fechaDiff);

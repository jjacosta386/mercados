 ...
 * The Class Derivados.
 */
public class DerivadosDrvx implements IDerivados{
	...
	public Set<Derivado> getDerivados(BusquedaDerivados busqueda)
		...
		if (BusquedaDerivados.isBusquedaVacia(busqueda) || BusquedaDerivados.isFechaHoy(busqueda)) {
			...
			while (itTipos.hasNext()) {
				...
				if (!tipoOpcion || (opcion.equalsIgnoreCase("E") && opcion.equalsIgnoreCase(busqueda.getTipoContrato()))){					
					Set<DerivadoResumen> res = derivadosDao.getDerivados("F", "F");
					if (noVacio(res)) {
						Derivado der = new Derivado();
						der.setSiglaDerivado(opcion);
						der.setNombreDerivado(getTiposContrato().get("E"));
			...
			if (!BusquedaDerivados.isBusquedaVacia(busqueda)
				...
				for (Iterator<DerivadoResumen> iterator2 = derivado
					...
					DerivadoResumen derivadoRes = (DerivadoResumen) iterator2
							.next();
					derivadoRes.setDerivadosExtendidos(
							derivadosDao.getDerivadosExtendidos(derivadoRes.getContrato(), busqueda));
				}
			}
		}
		return result;

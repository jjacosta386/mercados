...
<script type='text/javascript' src='/mercados/dwr/interface/derivadosAjax.js'></script>
...
//jQuery.noConflict();
    ...
    jQuery(document).ready
     (
                    function ( )
                    {
                        derivadosAjax.getNemotecnicos(getAnswer);
                    }
            );
    ...
    {
        array = answer;
        jQuery("#contrato").autocompleteArray(array, {delay: 10, minChars: 1, matchSubset: 1, onItemSelect: selectItem, onFindValue: findValue, autoFill: true, maxItemsToShow: 10});
    }
    ...
    function cambiarDerivados() {
        var obj = document.getElementById("derivTodas");
        var obj2 = document.getElementById("derivSuben");
        var obj3 = document.getElementById("derivBajan");

        var lista = document.getElementById("caso2");
...
<div id="tab_contenedor_mercados">
    <div class="tab_contenido_mercados">
        ...
        <form id="busqueda" action="<%=formAction%>" method="post" name="formDerivados">
            <div id="derivados" class="panelactivo_merc_divisas">
                <ul class="mercados">
                    ...
                    <li class="fechaneg">
                        <label>
                            <span id="text_2">Fecha de negociaci&oacute;n :</span>
                    ...
                    <li>
                        ...
                        <li>
                            <label><span id="text_4">Filtrar por :</span>
                                <select name="operacion" size="1" class="operacion" id="caso2" onChange="cambiarDerivados();">
                                    <option value="1">Los mas negociados</option>
                                    <option value="2">Los que suben</option>
                                    <option value="3">Los que bajan</option>
                                </select>
                            </label>
                        </li>
                        <li>
                    ...
                    <li class="fechaneg">
                        <div class="boton_regular"> <a href="javascript:document.formDerivados.submit();">Buscar</a></div>
                    </li>
                </ul>
            </div>
        </form>
...
<div class="titulo_seccion_unacolumna" id="text_11">
    <div class="subtitulo_mercados">Mercado de Derivados</div>
    <div class="subtitulo_mercados_pequeno"><c:out value="${estadoMercado}"/></div>
</div>
<div id="nota_titulo">
    <h3></h3>
    <h2>Informaci&oacute;n con <c:out value="15"></c:out> minutos de retraso</h2>
    </div>
    ...
    <div class="derivados_detalle_tabla_contenedor">
        <div class="derivados_detalle_tabla">
            <table cellpadding="0" cellspacing="0" class="tabla_general_derivados_detalle" id="text_12">
                <tr>
                    <th width="20%">Contratos</th>
                    <th width="15%">No. Contratos</th>
                    <th width="20%">Volumen (COP)</th>
                    <th width="15%">Open Interest</th>
                    <th width="15%">Participaci&oacute;n %</th>
                    <th width="15%">
                <div id="text_btn_ver" class="btn_ver" onclick="
                        MM_changeProp('tabla_derivados_superior', '', 'display', 'block', 'DIV');
                        MM_changeProp('text_btn_cerrar', '', 'display', 'block', 'DIV');
                        MM_changeProp('text_btn_ver', '', 'display', 'none', 'DIV');
                        ">
                    <a id="text_40" href="#">Ver detalles</a><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/flecha_inactivo.gif" alt="Ver" border="0" />
                </div>

                <div id="text_btn_cerrar" class="btn_cerrar" onclick="
                        MM_changeProp('tabla_derivados_superior', '', 'display', 'none', 'DIV');
                        MM_changeProp('text_btn_cerrar', '', 'display', 'none', 'DIV');
                        MM_changeProp('text_btn_ver', '', 'display', 'block', 'DIV');
                        ">
                    <a id="text_41" href="#">Cerrar</a><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/flecha_activo.gif" alt="Cerrar" border="0" />
                </div>
                </th>
                </tr>
            </table>
        </div>
        <div class="derivados_detalle_tabla" id="tabla_derivados_superior" <c:if test="${anioOPCF == '2011' }">style="visibility: hidden; display: none"</c:if>
                                                                           <c:if test="${anioOPCF != '2011' }">style="visibility: visible; display: block"</c:if>>
            <table cellpadding="0" cellspacing="0" class="tabla_general_derivados_detalle" id="text_15">
            <c:forEach items="${mercadoDerivados}" var="md">
                <c:if test="${md.contrato ne 'Total'}">
                    <tr>
                        <th width="20%" class="specblue"><c:out value="${md.contrato}"  /></th>
                        <td width="15%" align="right"><fmt:formatNumber type="number"><c:out value="${md.contratos}"  /></fmt:formatNumber></td>
                    <td width="20%" align="right"><fmt:formatNumber type="number"><c:out value="${md.volumen}"  /></fmt:formatNumber></td>
                        <c:if test="${md.contrato eq 'OPCF'}">
                        <td width="15%" align="center">&nbsp;</td>
                    </c:if>
                    <c:if test="${md.contrato ne 'OPCF'}">
                        <td width="15%" align="right"><fmt:formatNumber type="number"><c:out value="${md.openInterest}"  /></fmt:formatNumber></td>
                        </c:if>
                    <td width="15%" align="right"><fmt:formatNumber pattern="##0.00" ><c:out value="${md.participacion}"  /></fmt:formatNumber>%</td>
                    <td width="15%" align="center">&nbsp;</td>
                    </tr>
                </c:if>
            </c:forEach>
    ...
    <div class="derivados_detalle_tabla" id="tabla_derivados_superior2">
        <table class="tabla_general_derivados_detalle" id="text_28" cellpadding="0" cellspacing="0">
            <tbody>
                <c:forEach items="${mercadoDerivados}" var="md">
                    <c:if test="${md.contrato eq 'Total'}">
                        <tr>
                            <th class="specblue" width="20%" ><c:out value="${md.contrato}"  /></th>
                            <th class="specblue" width="15%" align="right"><fmt:formatNumber type="number"><c:out value="${md.contratos}"  /></fmt:formatNumber></th>
                    <th class="specblue" width="20%" align="right"><fmt:formatNumber type="number"><c:out value="${md.volumen}"  /></fmt:formatNumber></th>
                    <th class="specblue" width="15%" align="right"><fmt:formatNumber type="number"><c:out value="${md.openInterest}"  /></fmt:formatNumber></th>
                    <th class="specblue" width="15%" align="right"><fmt:formatNumber pattern="##0.00" ><c:out value="${md.participacion}"  /></fmt:formatNumber>%</th>
                    <th class="specblue" width="15%">&nbsp;</th>
                    </tr>
                </c:if>
            </c:forEach>
            </tbody>
...
<div class="contenedor_acordeon_borde" id="derivTodas" style="display:block;">
    <div class="contenedor_acordeon_sin_borde">
        <div id="acordeon_contenedor">
            <c:forEach items="${derivadosResumen}" var="dr" varStatus="status">
                <div class="acordeon_titulo_interior">
                    <div class="acordeon_titulo_segmento">
                        <p id="text_29">
                            <c:out value="${dr.nombreDerivado}"/>
                        </p>

                        <div id='text_ver_seg_<c:out value="${status.count}"/>' class="ver_detalles_bvc" onclick="
                                mostrar_segmento_acordeon('seg_acc_<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_cerrar_seg_<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_ver_seg_<c:out value="${status.count}"/>');
                             ">
                            Ver detalles
                        </div>
                        <div id="text_cerrar_seg_<c:out value="${status.count}"/>" class="cerrar_detalle_bvc" onclick="
                                ocultar_segmento_acordeon('seg_acc_<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_cerrar_seg_<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_ver_seg_<c:out value="${status.count}"/>');
                                ">
                            cerrar
                        </div>
                    </div>
                </div>
                <div id="seg_acc_<c:out value="${status.count}"/>" class="acordeon_interior_contenido">
                    <div class="renta_acordeon_interior">
                        <table align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="text_18">
                            <tr>
                                <th width="295px">Tipo de t&iacute;tulo</th>
                                <th width="90px">Cantidad</th>
                                <th width="250px">Volumen</th>
                                <th>&nbsp;</th>
                            </tr>
                        </table>
                    </div>
                    <div class="renta_acordeon_interior_renta">
                        <table width="100%" cellpadding="0" cellspacing="0"	class="tabla_general" id="text_17">
                            <tr>
                                <th width="20%">Instrumento <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                 alt="Drop" width="14" height="14" /></th>
                                <th width="10%">No. Contratos<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                  alt="Drop" width="14" height="14" /></th>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                    <th width="10%">Open Interest<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                      alt="Drop" width="14" height="14" /></th>
                                    </c:if>
                                <th width="20%">Volumen (COP)<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif" alt="Drop"
                                                                  width="14" height="14" /></th>
                                    <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                    <th width="20%">Volumen (MWh)<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif" alt="Drop"
                                                                      width="14" height="14" /></th>
                                    </c:if>
                                <th width="40%">&nbsp;</th>
                            </tr>
                    ...
                    <c:forEach items="${dr.derivadosResumen}" var="fr" varStatus="st">
                        <div id='futur_<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_acordeon_interior_renta">
                            <table width = "100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="textet_<c:out value="${st.count}"  />">
                                <tr>
                                    <th class="futurderivado" width="20%" style="text-align:left;"><c:out value="${fr.contrato}"  /></th>
                                    <th class="futurderivado" align="right" width="10%"><fmt:formatNumber type="number"><c:out value="${fr.contratos}"  /></fmt:formatNumber></th>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                    <th class="futurderivado" align="right" width="10%"><span class="alt"><fmt:formatNumber type="number"><c:out value="${fr.openInterest}"  /></fmt:formatNumber></span></th>
                                            </c:if>
                                <th class="futurderivado" align="right" width="20%"><fmt:formatNumber type="number"><c:out value="${fr.volumen}"  /> </fmt:formatNumber></th>
                                    <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                    <th class="futurderivado" align="right" width="20%"><fmt:formatNumber type="number"><c:out value="${fr.volumenMwh}"  /> </fmt:formatNumber></th>
                                    </c:if>
                                    <c:if test="${dr.nombreDerivado ne 'Futuros Energéticos'}">
                                    <th class="futurderivado" width="40%" align="center">
                                    <div id='cerrar_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn_cerrar"
                                         onclick="MM_changeProp('deuda_public<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('ver_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_inactivo('futur_<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                                 ">Cerrar
                                        <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                    <div id='ver_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn"
                                         onclick="MM_changeProp('deuda_public<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'block', 'DIV');
                                                 MM_changeProp('ver_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_activo('futur_<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                         ">Ver detalle <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                    </th>
                                </c:if>
                                <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                    <th class="futurderivado" width="20%" align="center">
                                    <div id='cerrar_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn_cerrar"
                                         onclick="MM_changeProp('deuda_public<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('ver_public<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                        ...
                        <div id='deuda_public<c:out value="${status.count }"/><c:out value="${st.count }"/>' class="renta_acordeon_interior">
                            <table width="100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id='texte_<c:out value="${status.count }"/><c:out value="${st.count }"/>'>
                                <tr>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="20%" align="left" class="specblue">Contrato</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="20%" align="left" class="specblue">Instrumento</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="10%" class="specblue">No. Contratos</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="10%" class="specblue">Vencimiento</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="10%" class="specblue">Open Interest</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="20%" class="specblue">No. Contratos</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="20%" class="specblue">Volumen</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="20%"  class="specblue">Volumen (COP)</th>
                                        </c:if>    
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                        <th width="20%" class="specblue">Volumen (MWh)</th>
                                        </c:if>


                                    <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="10%" class="specblue">&Uacute;ltimo Precio</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                        <th width="10%" class="specblue">&Uacute;ltimo Precio</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Financieros'}">
                                        <th width="20%" class="specblue">&Uacute;ltimo Precio</th>
                                        </c:if>

                                    <c:if test="${dr.nombreDerivado ne 'Futuros Financieros'}">
                                        <th width="10%"  class="specblue">Variaci&oacute;n</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Financieros'}">
                                        <th width="20%"  class="specblue">Variaci&oacute;n</th>
                                        </c:if>

                                </tr>
                                <c:forEach items="${fr.derivadosExtendidos}" var="dre" varStatus="stat">
                                    <tr>

                                    <portlet:actionURL var="linkDetalleDerivado">
                                        <portlet:param name="action" value="detalleDerivado" />
                                        <portlet:param name="opcf" value="otro" />
                                        <portlet:param name="nemo">
                                            <jsp:attribute name="value">
                                                <c:out value="${dre.contrato}"/>
                                            </jsp:attribute>
                                        </portlet:param>
                                    </portlet:actionURL>
                                    <portlet:actionURL var="linkDetalleOPCF">
                                        <portlet:param name="action" value="detalleDerivado" />
                                        <portlet:param name="opcf" value="opcf" />
                                        <portlet:param name="nemo">
                                            <jsp:attribute name="value">
                                                <c:out value="${dre.contrato}"/>
                                            </jsp:attribute>
                                    ...
                                    <portlet:actionURL var="linkDetalleDerivadoDrvx">
                                        <portlet:param name="action" value="detalleDerivadoDrvx" />
                                        <portlet:param name="opcf" value="otro" />
                                        <portlet:param name="nemo">
                                            <jsp:attribute name="value">
                                                <c:out value="${dre.contrato}"/>
                                    ...
                                    <td align="center" class="specblue">
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Financieros'}">
                                            <a href="<%=linkDetalleDerivado%>"><c:out value="${dre.contrato}"  /></a>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                            <a href="<%=linkDetalleDerivadoDrvx%>"><c:out value="${dre.contrato}"  /></a>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                            <c:out value="${fr.contrato}"  />
                                        </c:if>
                                    </td>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <td align="right">
                                        <fmt:formatNumber type="number"><c:out value="${dre.contratos}"  /></fmt:formatNumber>
                                        </td>
                                    </c:if>
                                    <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <td align="center">
                                            <a href="<%=linkDetalleOPCF%>"><c:out value="${dre.contrato}"  /></a>
                                        </td>
                                    </c:if>
                                    </td>
                                    <td align="right">
                                        <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <fmt:formatNumber type="number"><c:out value="${dre.openInterest}"  /></fmt:formatNumber>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <fmt:formatNumber type="number"><c:out value="${dre.contratos}"  /></fmt:formatNumber>
                                        </c:if>
                                    </td>
                                    <td align="right"><fmt:formatNumber type="number"><c:out value="${dre.volumen}"  /> </fmt:formatNumber></td>

                                    <c:if test="${dr.nombreDerivado eq 'Futuros Energéticos'}">
                                        <td align="right"><fmt:formatNumber type="number"><c:out value="${dre.volumenMwh}"  /> </fmt:formatNumber></td>
                                        </c:if>


                                    <td  align="right"><span class="alt"><fmt:formatNumber pattern="###,###.00" > <c:out value="${dre.precio}"  /> </fmt:formatNumber></span></td>
                                    <td  align="right"><fmt:formatNumber pattern="#0.00" > <c:out value="${dre.variacion}"  /> </fmt:formatNumber>%
                                        <c:choose>
                                            <c:when test="${dre.variacion gt 0}">
                                            <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/greenup.gif" alt="Sube" width="11" height="8" />
                                        </c:when>
                                        <c:when test="${dre.variacion eq 0}">
                                            <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/equal.gif" alt="Igual" width="11" height="8" />
                                        </c:when>
                                        <c:otherwise>
                                            <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/reddown.gif" alt="Baja" width="11" height="8" />
                                        </c:otherwise>
                                    </c:choose>
                                    </td>
                                    </tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="10">
            <tr>
                ...
                <td width="45%" align="right">
                    <a id="texto_1" href='/mercados/DescargaXlsServlet?archivo=derivados&anio=<c:out value="${busquedaDerivados.anioFecha}"/>&mes=<c:out value="${busquedaDerivados.mesFecha}"/>&dia=<c:out value="${busquedaDerivados.diaFecha}"/>' class="excelneg">
                        Descargar informaci&oacute;n de negociaci&oacute;n
                    </a>
                </td>
            </tr>
        </table>
...
<div class="contenedor_acordeon_borde" id="derivSuben" style="display:none;">
    <div class="contenedor_acordeon_sin_borde">
        <div id="acordeon_contenedor">
            <c:forEach items="${derivadosResumen}" var="dr" varStatus="status">
                <div class="acordeon_titulo_interior">
                    <div class="acordeon_titulo_segmento">
                        <p id="text_29"><c:out value="${dr.nombreDerivado}" /></p>
                        <div id='text_ver_seg_2<c:out value="${status.count}"/>' class="ver_detalles_bvc" onclick="
                                mostrar_segmento_acordeon('seg_acc_2<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_cerrar_seg_2<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_ver_seg_2<c:out value="${status.count}"/>');
                             ">
                            Ver detalles
                        </div>
                        <div id="text_cerrar_seg_2<c:out value="${status.count}"/>" class="cerrar_detalle_bvc" onclick="
                                ocultar_segmento_acordeon('seg_acc_2<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_cerrar_seg_2<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_ver_seg_2<c:out value="${status.count}"/>');
                                ">
                            cerrar
                        </div>
                    </div>
                </div>
                <div id="seg_acc_2<c:out value="${status.count}"/>" class="acordeon_interior_contenido">
                    <div class="renta_acordeon_interior">
                        <table align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="text_18">
                            <tr>
                                <th width="295px">Tipo de t&iacute;tulo</th>
                    ...
                    <div class="renta_acordeon_interior_renta">
                        <table width="100%" cellpadding="0" cellspacing="0"	class="tabla_general" id="text_17">
                            <tr>
                                <th width="120">Instrumento <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                 alt="Drop" width="14" height="14" /></th>
                                <th width="110">No. Contratos<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                  alt="Drop" width="14" height="14" /></th>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                    <th width="120">Open Interest<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif"
                                                                      alt="Drop" width="14" height="14" /></th>
                                    </c:if>
                                <th width="120">Volumen<img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/btn_drop.gif" alt="Drop"
                                                            width="14" height="14" /></th>
                                <th>&nbsp;</th>
                            </tr>
                        </table>
                    ...
                    <c:forEach items="${dr.derivadosResumen}" var="fr" varStatus="st">
                        <div id='futur_2<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_acordeon_interior_renta">
                            <table width="100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="textet_2<c:out value="${st.count}"  />">
                                <tr>
                                    <td width="20%"><c:out value="${fr.contrato}"  /></td>
                                    <td align="right" width="20%"><fmt:formatNumber type="number"><c:out value="${fr.contratos}"  /></fmt:formatNumber></td>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                    <td align="right" width="10%"><span class="alt"><fmt:formatNumber type="number"><c:out value="${fr.openInterest}"  /></fmt:formatNumber></span></td>
                                            </c:if>
                                <td align="right" width="20%"><fmt:formatNumber type="number"><c:out value="${fr.volumen}"  /> </fmt:formatNumber></td>



                                <td width="20%" align="center">
                                    <div id='cerrar_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn_cerrar"
                                         onclick="MM_changeProp('deuda_public2<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('ver_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_inactivo('futur_2<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                                 ">Cerrar
                                        <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                    <div id='ver_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn"
                                         onclick="MM_changeProp('deuda_public2<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'block', 'DIV');
                                                 MM_changeProp('ver_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public2<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_activo('futur_2<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                         ">Ver detalle <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                </td>
                                </tr>
                        ...
                        <div id='deuda_public2<c:out value="${status.count }"/><c:out value="${st.count }"/>' class="renta_acordeon_interior">
                            <table width="100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id='texte_2<c:out value="${status.count }"/><c:out value="${st.count }"/>'>
                                <tr>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="20%" class="specblue">Contrato</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado eq 'OPCF'}">
                                        <th width="20%" class="specblue">Instrumento</th>
                                        </c:if>
                                        <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="10%" class="specblue">No. Contratos</th>
                                        </c:if>
                                ...
                                <c:forEach items="${fr.derivadosExtendidos}" var="dre" varStatus="stat">
                                    <c:if test="${dre.variacion gt 0}">
                                        <tr>
                                        <portlet:actionURL var="linkDetalleDerivado">
                                            <portlet:param name="action" value="detalleDerivado" />
                                            <portlet:param name="opcf" value="" />
                                            <portlet:param name="nemo">
                                                <jsp:attribute name="value">
                                                    <c:out value="${dre.contrato}"/>
                                                </jsp:attribute>
    ...
    <div class="contenedor_boton_imprimir">
        <table width="100%" border="0" cellspacing="0" cellpadding="10">
            <tr>
                <td width="45%">&nbsp;</td>
                <td width="10%">&nbsp;</td>
                <td width="45%" align="right"><a href='/mercados/DescargaXlsServlet?archivo=derivados&anio=<c:out value="${busquedaDerivados.anioFecha}"/>&mes=<c:out value="${busquedaDerivados.mesFecha}"/>&dia=<c:out value="${busquedaDerivados.diaFecha}"/>' class="excelneg">Descargar
                        informaci&oacute;n de negociaci&oacute;n</a></td>
            </tr>
        </table>
    </div>
</div>
<div class="contenedor_acordeon_borde" id="derivBajan" style="display:none;">
    <div class="contenedor_acordeon_sin_borde">
        <div id="acordeon_contenedor">
            <c:forEach items="${derivadosResumen}" var="dr" varStatus="status">
                <div class="acordeon_titulo_interior">
                    <div class="acordeon_titulo_segmento">
                        <p id="text_29"><c:out value="${dr.nombreDerivado}" /></p>
                        <div id='text_ver_seg_3<c:out value="${status.count}"/>' class="ver_detalles_bvc" onclick="
                                mostrar_segmento_acordeon('seg_acc_3<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_cerrar_seg_3<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_ver_seg_3<c:out value="${status.count}"/>');
                             ">
                            Ver detalles
                        </div>
                        <div id="text_cerrar_seg_3<c:out value="${status.count}"/>" class="cerrar_detalle_bvc" onclick="
                                ocultar_segmento_acordeon('seg_acc_3<c:out value="${status.count}"/>');
                                ocultar_segmento_acordeon('text_cerrar_seg_3<c:out value="${status.count}"/>');
                                mostrar_segmento_acordeon('text_ver_seg_3<c:out value="${status.count}"/>');
                                ">
                            cerrar
                        </div>
                    </div>
                </div>
                <div id="seg_acc_3<c:out value="${status.count}"/>" class="acordeon_interior_contenido">
                    <div class="renta_acordeon_interior">
                        <table width = "100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="text_18">
                            <tr>
                                <th width="295px">Tipo de t&iacute;tulo</th>
                                <th width="90px">Cantidad</th>
                                <th width="250px">Volumen</th>
                    ...
                    <c:forEach items="${dr.derivadosResumen}" var="fr" varStatus="st">
                        <div id='futur_3<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_acordeon_interior_renta">
                            <table align="left" cellpadding="0" cellspacing="0" class="tabla_general" id="textet_3<c:out value="${st.count}"  />">
                                <tr>
                                    <td width="120"><c:out value="${fr.contrato}"  /></td>
                                    <td align="right" width="110"><fmt:formatNumber type="number"><c:out value="${fr.contratos}"  /></fmt:formatNumber></td>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                    <td align="right" width="120"><span class="alt"><fmt:formatNumber type="number"><c:out value="${fr.openInterest}"  /></fmt:formatNumber></span></td>
                                            </c:if>
                                <td align="right" width="120"><fmt:formatNumber type="number"><c:out value="${fr.volumen}"  /> </fmt:formatNumber></td>
                                <td align="center">&nbsp;</td>
                                <td width="120" align="center">
                                    <div id='cerrar_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn_cerrar"
                                         onclick="MM_changeProp('deuda_public3<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('ver_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_inactivo('futur_3<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                                 ">Cerrar
                                        <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                    <div id='ver_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />' class="renta_btn"
                                         onclick="MM_changeProp('deuda_public3<c:out value="${status.count}"  /><c:out value="${st.count }"/>', '', 'display', 'block', 'DIV');
                                                 MM_changeProp('ver_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'none', 'DIV');
                                                 MM_changeProp('cerrar_public3<c:out value="${status.count}"  /><c:out value="${st.count}"  />', '', 'display', 'block', 'DIV');
                                                 mostrar_campo_interno_activo('futur_3<c:out value="${status.count}"  /><c:out value="${st.count}"  />');
                                         ">Ver detalle <img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/ico_cerrar.gif" alt="Cerrar" border="0" />
                                    </div>
                                </td>
                                </tr>
                        ...
                        <div id='deuda_public3<c:out value="${status.count }"/><c:out value="${st.count }"/>' class="renta_acordeon_interior">
                            <table width = "100%" align="left" cellpadding="0" cellspacing="0" class="tabla_general" id='texte_3<c:out value="${status.count }"/><c:out value="${st.count }"/>'>
                                <tr>
                                    <c:if test="${dr.nombreDerivado ne 'OPCF'}">
                                        <th width="20%" align="left" class="specblue">Contrato</th>
                                        </c:if>

...
<portlet:renderURL var="buscarTodosAction">
	<portlet:param name="action" value="listarTodos" />   
</portlet:renderURL>
...
(       
        ...
        function( ) 
	{
		accionesMGCAjax.getEmisores( getAnswerEmi );
	}
);


        var getAnswerEmi = function( answer )
        {
        array = answer;
        jQuery("#nombreEmisor").autocompleteArray ( array, { delay:10, minChars:1, matchSubset:1, onItemSelect:selectItem, onFindValue:findValue, autoFill:true, maxItemsToShow:10 } );
        }


</script>

<div class="titulo_seccion_lisemi" id="texto_1">Listado de Acciones y ETFs - Mercado Global Colombiano</div>
	<div id="texto_2" class="text_contenido_general">
    </div>
  	
    <div class="boton_buscador_comisionistas">
  		<div class="boton_buscador_comisionistas_titulo" id="texto_3" style="padding-right:5px; padding-left:5px; margin:0px;">B&uacute;squeda de Acciones y ETFs</div>
		
  		<div class="buscador_comisionistas_cerrar" onclick="MM_effectBlind('buscador_comisionistas', 500, '100%', '0%', true)" onmouseover="MM_showHideLayers('buscador_comisionistas','','show')" onkeydown="MM_showHideLayers('buscador_comisionistas','','show')">
	  		<a href="#"><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/flecha_activo.gif" alt="Cerrar"/></a>
	  	</div>
  	</div>
  	<div id="buscador_comisionistas" class="buscador_emisores">
     	<forM id="formularioBusqueda" name="formularioBusqueda" method="post" action="<%=formBuscarAction%>">
        	<table width="100%" cellspacing="10" cellpadding="0" border="0">
          		<tr>
          			<td class="td1_comisionista" id="texto_4" style="width: 82px">Tipo de Instrumento: </td>
					<td align="left">
						<select name="tipoMercado" size="1" class="operacion" id="tipoMercado" style="width: 120px; margin: 0px; padding: 0px">
						<option value="0" <c:if test="${formularioBusqueda.tipoMercado == '0'}">selected="selected"</c:if>>
						Todos</option>
							<option value="4" 
								<c:if test="${formularioBusqueda.tipoMercado == '4'}">selected="selected"</c:if>>
								Acciones
							</option>
							<option value="5"
								<c:if test="${formularioBusqueda.tipoMercado == '5'}">selected="selected"</c:if>>
								ETFs
							</option>
							
						</select>	
					</td>
					<td class="td1_comisionista" id="texto_6">Fecha de Inscripci&oacute;n:</td>
				  ...
				  <td style="padding:4px 0px">
					 <label style="width: auto;">
				        <span class="fechalista" style="padding-left:10px">	
				  ...
				  <td style="padding:4px 0px">
                     <label style="width: auto;">
                         <span class="fechalista" style="padding-left:10px">
                                <select name="anioFecha" size="1" id="anioFecha" style="width: 40px">
					...
					<td id="texto_8" style="width: 100px">
	            		<div class="td1_comisionista" id="opcion1" style="width: auto">Patrocinador:</div>
	            	</td>
            		<td align="left">
        	    		<select name="patrocinador" size="1" class="operacion" id="patrocinador" style="width:120px; margin:0px; padding:0px">
          	   				<option value="%">Todos</option>
            				<c:forEach items="${listaPatrocinador}" var="Patr">
            					<option value="<c:out value="${Patr.patrocinador}"/>"
									<c:if test="${formularioBusqueda.patrocinador ==Patr.patrocinador}">
										selected="selected"
									</c:if>
								>
						    		<c:out value="${Patr.patrocinador}"/>
								</option>
            				</c:forEach>
          				</select>
				...
				<table width="100%" cellspacing="10" cellpadding="0" border="0">
				<tr>
				<td class="td1_comisionista" id="texto_10" style="width: 83px">
            			<label>Emisor:</label>
            		</td>
            		<td align="left" id="texto_11">
              			<input type="text" value='<c:out value="${formularioBusqueda.nombreEmisor}"/>' name="nombreEmisor" id="nombreEmisor" style="width:117px; margin:0px; padding:0px" />
            		</td>
            		
                 <td class="td1_comisionista" id="texto_12" style="width: 118px;">Cotizaci&oacute;n <br></br>Principal Inicial:</td>
					<td align="left">
						<select name="cotizacion" size="1" class="operacion" id="cotizacion"  style="width:160px; margin:0px; padding:0px" >
							<option value="%">Todos</option>
							<c:forEach items="${listaCotiza}" var="coti">
								<option value="<c:out value="${coti.cotizacion}"/>"
									<c:if test="${formularioBusqueda.cotizacion ==coti.cotizacion}">
										selected="selected"
									</c:if>
									>
									<c:out value="${coti.cotizacion}"/>
								</option>
							</c:forEach>
						</select>
						</td>

					<td class="td1_comisionista" id="texto_14">País de Origen:</td>
					<td align="left">
						<select name="pais" size="1" class="operacion" id="pais"  style="width:120px; margin:0px; padding:0px" >
							<option value="%">Todos</option>
							<c:forEach items="${listaPaises}" var="pais">
							<option value="<c:out value="${pais.pais}"/>"
									<c:if test="${formularioBusqueda.pais ==pais.pais}">
										selected="selected"
									</c:if>
								>
        	...
        	<table width="100%" cellpadding="0" cellspacing="10" border="0">
          	<tr>
            	<td class="td2_comisionista">&nbsp;</td>
            	<td width="70" class="td2_comisionista">
            		<div class="boton_regular">
              			<a href="javascript:document.formularioBusqueda.submit();">Buscar</a>
             			<!--  <input type="submit" value="Buscar" />  --> <!--a href="#">Buscar</a-->
             		</div>
            	</td>
            	<td width="150" align="right" class="td2_comisionista">
            		<!--<div class="buscador_comisionista_lista_completa" id="texto_16"><a href="<%=buscarTodosAction%>">Ver lista completa</a></div>-->
            	</td>
            	<td class="td2_comisionista"></td>
          	</tr>
    	</table>

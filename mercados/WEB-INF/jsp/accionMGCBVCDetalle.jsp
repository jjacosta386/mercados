...
<script type='text/javascript' src='/mercados/dwr/interface/accionesMGCAjax.js'></script>
...
(       
        ...
        function( ) 
	{
		accionesMGCAjax.getNemotecnicos( getAnswer );
	}
);
        ...
        {
        array = answer;
        jQuery("#nemo").autocompleteArray ( array, { delay:10, minChars:1, matchSubset:1, onItemSelect:selectItem, onFindValue:findValue, autoFill:true, maxItemsToShow:10 } );
        }

	function enviarFormularioDesdeEnlace(codigo){
		ele = document.getElementById('nemo');
		ele.value = codigo;
		return true; 		
	}
	...
	function cambiarAcciones() {
		var obj = document.getElementById("tbAcNegociadas");
		var obj2 = document.getElementById("tbAcSube");
		var obj3 = document.getElementById("tbAcBaja");
...
<div id="tab_contenedor_mercados">
 <div class="tab_contenido_mercados">
		...
		<form id="formulario" name="busqueda" action="<%=formAction%>" method="post">
		
		  <div id="acciones" class="panelactivo_merc_divisas">
			<ul class="mercados">
				<li style="height:25px;">
					<label>
						...
						<select name="tipoMercado" size="1" class="operacion" id="tipoMercado" style="width: 153px">
							<option value="1" 
								<c:if test="${formulario.tipoMercado == '1'}">selected="selected"</c:if>>
								Compraventas
							</option>
							<option value="2"
								<c:if test="${formulario.tipoMercado == '2'}">selected="selected"</c:if>>
								Repos
							</option>
							<option value="3"
								<c:if test="${formulario.tipoMercado == '3'}">selected="selected"</c:if>>
								TTV
							</option>
						</select>
					</label>
				</li>
				<li class="fechaneg" style="height:25px; width:390px">
					<label style="width: auto;">
						<span id="texto_12" style="width:140px;">Fecha de negociación:</span>
						<span class="fechalista">
						          <select name="diaFecha" size="1" id="diaFecha" style="width: 40px">
										<c:forEach items="${dias}" 
										var="dia">
											<option
												<c:if test="${formulario.diaFecha==dia.key}">selected="selected"</c:if>
												value='<c:out value="${dia.key}"/>'><c:out value="${dia.value}" />
											</option>
										</c:forEach>
									</select>
						</span>	
					 ...
					 <label style="width: auto;">
				        <span class="fechalista">	
				                <select name="mesFecha" size="1" id="mesFecha" style="width: 40px">
										<c:forEach items="${meses}" var="mes">
											<option
												<c:if test="${formulario.mesFecha==mes.key}">selected="selected"</c:if>
												value='<c:out value="${mes.key}"/>'><c:out value="${mes.value}" />
											</option>
										</c:forEach>
								</select>
						</span>
                     ...
                     <label style="width: auto;">
                         <span class="fechalista">
                                <select name="anioFecha" size="1" id="anioFecha" style="width: 60px">
										<c:forEach items="${anios}" var="anio">
											<option
												<c:if test="${formulario.anioFecha==anio.key}">selected="selected"</c:if>
												value='<c:out value="${anio.key}"/>'><c:out value="${anio.value}" />
											</option>
										</c:forEach>
								</select>
                         </span>
				...
				<li class="fechaneg" style="height:25px; width:auto">
					<c:if test="${resumen}">
						<c:if test="${tipoMercado==null || tipoMercado == 1}">
							<label>
								<span id="text_14">Filtrar por :</span>
						<%try{%>
<select class="operacion" size="1" onChange="cambiarAcciones();" id="filtroAcciones" >
<%if (request.getParameter("filtrarPor") == null || request.getParameter("filtrarPor").equals("masVendidas") ){%>
									<option value="1" selected>Las mas negociadas</option>
									<option value="2">Las que suben</option>
									<option value="3">Las que bajan</option>
								
<%} else if (request.getParameter("filtrarPor") != null && request.getParameter("filtrarPor").equals("suben")){%>
<option value="1">Las mas negociadas</option>
									<option value="2" selected>Las que suben</option>
									<option value="3">Las que bajan</option>
<%} else if (request.getParameter("filtrarPor") != null && request.getParameter("filtrarPor").equals("bajan")){%>
<option value="1">Las mas negociadas</option>
									<option value="2">Las que suben</option>
									<option value="3" selected>Las que bajan</option>
<%} else{ %>
<option value="1" selected>Las mas negociadas</option>
									<option value="2">Las que suben</option>
									<option value="3">Las que bajan</option>
<%}%>
					...
					<div id="contenedor_buscador">
						<div class="boton_regular"><a href="javascript:document.busqueda.submit();">Buscar</a></div>
					</div>
				</li>
			</ul>
			</div>
...
<div id="nota_titulo">
<h3>&nbsp;</h3>
<h2 id="texto_9">Informaci&oacute;n en l&iacute;nea</h2>
</div>
<br clear="all" />
	...
	<div class="tabber" title="gráfica">
		<div class="tabbertab"  title="Diaria">
			<div id="texto_grafica_dia" style="height: 100%">
				<script type="text/javascript">
					var bvcGraficoDia = new SWFObject("%%SiteContent.Portlet('common.bvc.recursos')%%/amstock/amstock.swf",
							"amstock", "100%", "370", "8", "#FFFFFF");
					bvcGraficoDia.addVariable("path", "");
					bvcGraficoDia.addParam("wmode","opaque");
					bvcGraficoDia
							.addVariable(
									"settings_file",
									escape("/mercados/GraficosServlet?conf=<c:out value='${titulo.nemotecnico}'/>&url=/amstock/settings/acciones_hoy_stock_settings.xml&tipoG=BVC&home=no&tiempo=hoy"));
					bvcGraficoDia.addVariable("chart_id", "amstockDia");
					bvcGraficoDia.addVariable("preloader_color", "#999999");
					licenciaGrafica(bvcGraficoDia);
					bvcGraficoDia.write('texto_grafica_dia');
				</script>
		...
		<div class="tabbertab"  title="Histórica">
			<div id="texto_grafica_hist" style="height: 100%">
				<script	type="text/javascript">
					var bvcGraficoHist = new SWFObject("%%SiteContent.Portlet('common.bvc.recursos')%%/amstock/amstock.swf",
							"amstock", "100%", "370", "8", "#FFFFFF");
					bvcGraficoHist.addVariable("path", "");
					bvcGraficoHist.addParam("wmode","opaque");
					bvcGraficoHist
							.addVariable(
									"settings_file",
									escape("/mercados/GraficosServlet?conf=<c:out value='${titulo.nemotecnico}'/>&url=/amstock/settings/acciones_stock_settings.xml&tipoG=BVC&home=no&tiempo=ayer"));
					bvcGraficoHist.addVariable("chart_id", "amstock");
					bvcGraficoHist.addVariable("preloader_color", "#999999");
					licenciaGrafica(bvcGraficoHist);
					bvcGraficoHist.write('texto_grafica_hist');
				</script>
			</div>
		</div>
	</div>
...
<p id="texto_23" style="width: 400px">Mercado Global - Resumen del Mercado - Compraventas</p>
...
<table cellpadding="0" cellspacing="0" class="tabla_basica"
	...
	<tbody>
		...
		<tr>
			<th class="specblue"><c:out value="${titulo.nemotecnico}" /></th>
			...
			<td align="right" style="padding-left: 3px;"> 
				<fmt:formatNumber pattern="###,##0.00">
					<c:out value="${titulo.variacion}" /></fmt:formatNumber>%<c:choose>
					<c:when test="${titulo.variacion>0}"><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/up.gif" alt="Sube"/>
					</c:when>
					<c:when test="${titulo.variacion<0}"><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/down.gif" alt="Baja"/>
					</c:when>
					<c:otherwise><img src="%%SiteContent.Portlet('common.bvc.recursos')%%/images/equal.gif" alt="Igual"/>					
					</c:otherwise>
				</c:choose>
			</td>
			<td align="right" style="padding-left: 3px;"><fmt:formatNumber pattern="###,##0.00">
...
<form id="formhistorico" action="/mercados/DescargaMGCXlsServlet"
	method="get" name="formhistorico"><input type="hidden"
	name="archivo" value="acciones_detalleMGC" /> <input type="hidden"
	name="nemo" value="<c:out value='${titulo.nemotecnico}'/>" /> <input
	type="hidden" name="tipoMercado"
	value="<c:out value='${tipoMercado}'/>" />
<ul id="text_19">
